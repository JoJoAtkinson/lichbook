...
| Plug in while risen, lid closed, machine asleep | *Should* wake enough to tick and go awake — but the watcher does not run while asleep, and whether the AC-attach dark-wake gives it a tick before re-sleeping is not knowable from this code | **Unknown** | Untested |
...
| Another app holds `caffeinate`-style IOPMAssertions at the same time | Coexists. `sleep_disabled()` reads the `SleepDisabled` line, not assertions, so it is not confused by them *(observed: two unrelated `caffeinate` assertions alongside `SleepDisabled 1`)* | Yes — different mechanism (lich:69-73) | Should-work |
...
| Screen Sharing to the **login window** (virtual display session) | A virtual session may create a `gui/$UID` domain and start the agent. Tahoe reportedly behaves differently when the target is at a login prompt rather than a live session *(unverified — conflicting reports)*. Note lich locks the console on every lid close, so the first connect after a lid close always faces authentication | **Unknown** | Untested |
...
| Clone directory moved/deleted after install | CLI symlink dangles and the agent's program path is gone | **No** — nothing detects it | Known-gap |
...
- **Q6 — Login Items & Extensions.** When a user has toggled the agent off
...
- **Q7 — Install/`on` honesty vs. the fail-open policy.** Should `lich install`
  exit non-zero when the agent did not load, and should `lich on` print an error
...
| Tahoe 26 lock-screen regressions: exiting the screensaver inside the password grace period shows a lock image without prompting; lid close/reopen inside a 5-minute grace window lands on the live desktop | degrades | discussions.apple.com 256165873; dev forums 787444 | Raises the odds that "screensaver started" ≠ "locked" even when set to Immediately. lich cannot detect it — it never re-checks after firing (§4) |
