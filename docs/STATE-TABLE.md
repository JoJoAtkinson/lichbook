...
| Sleep + DarkWake continuity: machine sleeps ~3h on battery with periodic maintenance DarkWakes | The watcher stays silent through the whole sleep (no ticks, no `disablesleep` flapping, no log lines), DarkWakes change nothing, and the first real wake resumes correct behavior immediately | Yes — field-verified | **Tested** 2026-08-02: 1-min heartbeat showed sleep from ~17:48 to 20:38 with 5 isolated DarkWake blips (25–44 min apart, never consecutive); lich log empty for the entire span; battery still 100% after ~3h unplugged |
