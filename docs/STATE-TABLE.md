...
| Second user's install clobbered the first user's sudoers rule (shared `/etc/sudoers.d/lichbook`), sending the first watcher into the EX_CONFIG respawn loop | **Fixed in v0.3.2** — per-user filenames (`lichbook-<user>`); install migrates away from the legacy shared file, uninstall removes both | Found during the 2026-08-03 multi-user think-through, not by testing. Multi-user remains officially one-owner-per-machine (§10), but installs are no longer mutually destructive |
